<script lang="ts">
  ...
  type AdminView = 'profile' | 'pages' | 'posts' | 'promotions' | 'appearance' | 'account';
  ...
  const currentEntityMeta = $derived.by(() => {
    ...
    if (view === 'account') return 'Theme and account preferences';
    return 'Content';
  });
  ...
  const currentSectionBlurb = $derived.by(() => {
    ...
    if (view === 'appearance') return 'Adjust reusable brand tokens before developer handoff.';
    return 'Manage account settings and dashboard theme.';
  });
  ...
  const sidebarItemBase = 'admin-rail-link';
  const sidebarItemActive = 'active';
  const businessDisplayName = $derived.by(() => {
    const name = draft?.site?.title?.trim();
    return name && name.length > 0 ? name : (appPkg.name || 'Local Business');
  });
</script>
...
{:else}
  <div class="admin-theme p-0 md:p-0" class:is-dark={darkMode}>
      {#if showToast}
      ...
      <div class="grid h-screen gap-0 overflow-hidden lg:grid-cols-[250px_1fr]">
        <aside class="admin-rail flex h-screen flex-col overflow-hidden">
          <div class="border-b px-2 pb-4 pt-2" style={`border-color: #e3e7ef;`}>
            <p class="m-0 text-[32px] font-semibold leading-none tracking-[-0.02em] text-[var(--admin-text-strong)]">{businessDisplayName}</p>
          </div>

          <div class="min-h-0 flex-1 overflow-y-auto pr-1">
            <p class="admin-rail-section-title">General</p>
            <nav class="grid gap-1">
              <button class={`${sidebarItemBase} ${view === 'profile' ? sidebarItemActive : ''}`} onclick={() => (view = 'profile')}><span class="admin-rail-icon">◎</span> Dashboard</button>
              <button class={`${sidebarItemBase} ${view === 'posts' ? sidebarItemActive : ''}`} onclick={() => (view = 'posts')}><span class="admin-rail-icon">✎</span> Posts</button>
              <button class={`${sidebarItemBase} ${view === 'promotions' ? sidebarItemActive : ''}`} onclick={() => (view = 'promotions')}><span class="admin-rail-icon">★</span> Promotions</button>
              <button class={`${sidebarItemBase}`}><span class="admin-rail-icon">◷</span> Hours</button>
              <button class={`${sidebarItemBase}`}><span class="admin-rail-icon">▣</span> Photos</button>
            </nav>

            <p class="admin-rail-section-title">Site</p>
            <nav class="grid gap-1">
              <button class={`${sidebarItemBase} ${view === 'profile' ? sidebarItemActive : ''}`} onclick={() => (view = 'profile')}><span class="admin-rail-icon">◉</span> Site Profile</button>
              <button class={`${sidebarItemBase} ${view === 'pages' ? sidebarItemActive : ''}`} onclick={() => (view = 'pages')}><span class="admin-rail-icon">□</span> Pages</button>
              <button class={`${sidebarItemBase} ${view === 'appearance' ? sidebarItemActive : ''}`} onclick={() => (view = 'appearance')}><span class="admin-rail-icon">◐</span> Appearance</button>
              <button class={`${sidebarItemBase}`}><span class="admin-rail-icon">≣</span> Menu</button>
              <button class={`${sidebarItemBase}`}><span class="admin-rail-icon">S</span> Settings & SEO</button>
            </nav>

            <p class="admin-rail-section-title">Workspace</p>
            <nav class="grid gap-1">
              <button class={`${sidebarItemBase} ${view === 'account' ? sidebarItemActive : ''}`} onclick={() => (view = 'account')}><span class="admin-rail-icon">◌</span> Account</button>
              <button class={`${sidebarItemBase}`}><span class="admin-rail-icon">?</span><GuideIcon className="h-4 w-4 shrink-0" />Guide</button>
              <button class={`${sidebarItemBase}`}><span class="admin-rail-icon">!</span><SupportIcon className="h-4 w-4 shrink-0" />Support</button>
              <a class={`${sidebarItemBase} no-underline`} href="/" target="_blank" rel="noreferrer"><span class="admin-rail-icon">↗</span><OpenSiteIcon className="h-4 w-4 shrink-0" />Open site</a>
            </nav>
          </div>

          <button class="mt-4 rounded-2xl border p-3 text-left" style={`border-color: var(--admin-panel-border); background: #f0f2f6;`} onclick={() => (view = 'account')}>
            <p class="m-0 text-sm font-semibold text-[var(--admin-text-strong)]">Your Symballo Account</p>
            <p class="m-0 mt-2 text-[28px] leading-none text-[var(--admin-text-strong)]">⚙</p>
            <p class="m-0 mt-2 text-sm text-[var(--admin-text-strong)]">Account Settings</p>
          </button>
        </aside>

        <section class="admin-panel ml-0 mr-3 mt-3 mb-3 flex h-[calc(100vh-1.5rem)] max-w-full min-w-0 flex-col overflow-hidden p-0">
          <header class="flex items-center justify-between border-b px-5 py-3" style={`border-color: var(--admin-panel-border);`}>
            <p class="m-0 text-[34px] leading-none text-[var(--admin-text-strong)]">Dashboard</p>
          ...
          <header class="mb-4 flex flex-wrap items-start justify-between gap-3 border-b pb-4" style={`border-color: var(--admin-panel-border);`}>
            <div>
              <h1 class="m-0 text-[56px] leading-[0.95] font-semibold text-[var(--admin-text-strong)]">{currentSectionName}</h1>
              <p class="mt-1 text-sm text-[var(--admin-text-soft)]">{currentSectionBlurb}</p>
              <p class="mt-1 text-xs text-[var(--admin-text-soft)]">{currentEntityMeta}</p>
            </div>
            <div class="flex items-center gap-2">
              <button
      ...
      {#if view === 'account'}
        <section class="admin-panel grid gap-4">
          <div>
            <h2 class="m-0 text-2xl text-[var(--admin-text-strong)]">Account Settings</h2>
            <p class="mt-2 text-sm text-[var(--admin-text-soft)]">Manage account preferences and editor theme.</p>
          </div>
          <div class="rounded-lg border p-4" style={`border-color: var(--admin-panel-border); background: var(--admin-panel-bg);`}>
            <p class="m-0 mb-3 text-sm font-semibold text-[var(--admin-text-strong)]">Theme mode</p>
            <div
