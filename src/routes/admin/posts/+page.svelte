<script lang="ts">
  ...
  async function deletePost(slug: string) {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('show-toast', { detail: 'Post deleted' }));
    } catch (err) {
      window.dispatchEvent(new CustomEvent('show-toast', { detail: `Delete failed: ${err instanceof Error ? err.message : 'unknown'}` }));
    }
  }
